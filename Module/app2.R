...
server <- function(input, output, session) {
  ...
  observeEvent(tolistenbuttons(), {
    ...
    output$Chart <- renderUI({
      ...
      tagList(
        fluidRow(p(fragezeile() %>% select(Info) %>% pull())), #Info zur Statistik
        fluidRow(
          selectInput(inputId = "Jahr",
                      label = "Jahr auswählen", 
                      choices = df %>% filter(Statistik_Code == selectedfragen()[i()], 
                                              is.na(Zwei_Code)) %>% 
                                                select(Zeit) %>% 
                                                unique() %>% 
                                                pull()),
          output$p <- renderPlot({
            ggplot(df %>% filter(Statistik_Code == selectedfragen()[i()], Zeit == input$Jahr, is.na(Zwei_Code)),
                   aes(x = AGS_Label, y = Wert)) + geom_col()+theme_bw()
          }, res = 96)
        ),
        fluidRow(
          actionButton(
